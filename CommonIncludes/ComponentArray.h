...
public:
	...
	~ComponentArray() {
		if (pointer != 0) {
			LOGGER::ECS->warn("%d components have not been removed.", pointer);
		}
	...
	void insertComponent(Entity entity, T component){
		if (entityToIndexMap.find(entity)!= entityToIndexMap.end()) {
			LOGGER::ECS->warn("Trying to insert component more than once for entity %i.",entity);
			return;
		}

		if (pointer > m_maxComponents - 1) {
			LOGGER::ECS->warn("Cannot add component, component array is full.");
			return;
		}
	...
	void removeComponent(Entity entity){
		if (entityToIndexMap.find(entity) == entityToIndexMap.end()) {
			LOGGER::ECS->warn("Trying to remove nonexistent component for entity %i.", entity);
			return;
		}

		if (pointer < 1) {
			LOGGER::ECS->warn("Cannot remove component for entity %i.",entity);
			return;
		}
	...
	T& getComponent(Entity entity){
		if (entityToIndexMap.find(entity) == entityToIndexMap.end()) {
			LOGGER::ECS->warn("Component does not exist for entity %i.", entity);
			return T();
		}
