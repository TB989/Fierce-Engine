...
#include "src/InputBindings.h"
#include "src/InputContext.h"

namespace Fierce {

	std::unordered_map<int, BINDING> WindowSystem::m_bindings = {
		// Function keys
	...
	LRESULT CALLBACK wndProcFierceWindow(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
		...
		case WM_SIZE:
			...
			//*** Keyboard ***//
		case WM_KEYDOWN:
			inputSystem->getActiveContext()->onKeyDown(WindowSystem::m_bindings[wParam]);
			return 0;
		case WM_KEYUP:
			inputSystem->getActiveContext()->onKeyUp(WindowSystem::m_bindings[wParam]);
			return 0;
		case WM_CHAR:
			return 0;
			//*** Mouse ***//
